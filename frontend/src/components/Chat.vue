...
<template>
  <div class="chat-box">
    ...
    <div id="comment" class="comment">
      <p>Based on Gemini Pro <a href="https://makersuite.google.com/app/apikey" target="_blank">API</a>.</p>
    </div>
    ...
    <div id="input" class="input-box ">
      ...
      <button class="btn" @click="send">
        <template v-if="sending">
          <svg focusable="false" data-icon="stop" width="1em" height="1em" fill="currentColor" aria-hidden="true"
        ...
        <template v-else>
          <span role="img" aria-label="send" class="anticon anticon-send"><svg focusable="false" data-icon="send"
      ...
      <button class="clr" title="Clear" gen-slate-btn="" @click="clear">
  ...
  <div class="setting">
    <button class="btn" @click="hidden_setting">
      <svg focusable="false" class="" data-icon="setting" width="1em" height="1em" fill="currentColor" aria-hidden="true"
        viewBox="64 64 896 896">
        <path
...
function send() {
  ...
  SendMessage(question.value, api_key.value, proxy.value).then((res: any) => {
    if (res.Code == 200) {
      ...
      let mg: Message = { id: messages.list[messages.list.length - 1].id + 1, sender: "robot", text: "No respond!!!!" }
      messages.list.push(mg)
      sending.value = !sending.value
    }
  })
